...
class MoviesNotifier extends StateNotifier {
  ...
  Future<void> addMovie(List<Post> equipos) async {
    final doc = db.collection('campeones-del-mundial').doc('campeones-del-mundial');
    try {
  ...
  Stream<List<Post>> getMovies() {
    return db.collection('campeones-del-mundial').doc('campeones-del-mundial').snapshots().map((snapshot) {
      posts = [];
